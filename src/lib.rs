...
pub fn run_decompile(options: DecompileOptions) -> Result<()> {
        ...
        match result {
            Ok(report) => {
                ...
                if report.summary.object_count > 0 {
                    print_model_summary("model", &report.summary);
                }
            }
            Err(err) => {
                failures += 1;
...
pub fn run_inspect(input: &Path) -> Result<()> {
    ...
    if let Some(xml) = model_xml {
        ...
        print_model_summary("model summary", &summary);
    } else {
...
fn open_archive(input: &Path) -> Result<ZipArchive<BufReader<File>>> {
    let file =
    ...
    if output.exists() {
        if overwrite {
            fs::remove_dir_all(output).with_context(|| {
                format!(
                    "failed removing existing output directory {}",
                    output.display()
                )
            })?;
        } else {
...
fn print_model_summary(label: &str, summary: &ModelSummary) {
    println!(
        "{label}: objects={}, mesh_objects={}, component_objects={}, build_items={}, metadata={}, unit={}",
...
mod tests {
    ...
    #[test]
    fn output_folder_name_replaces_spaces() {
        assert_eq!(
            output_folder_name(Path::new("C:/tmp/my model.3mf")),
            "my_model"
        );
    }
    ...
    fn safe_output_path_rejects_parent_components() {
        ...
        let result = safe_output_path(base, "../escape.txt");
        assert!(result.is_err());
    }

    #[test]
    fn safe_output_path_builds_nested_path() {
        let base = Path::new("out");
        let result = safe_output_path(base, "3D/3dmodel.model").unwrap();
        assert_eq!(result, base.join("3D").join("3dmodel.model"));
    }

    #[test]
    fn xml_file_detection_is_case_insensitive() {
        assert!(is_xml_file("META/metadata.XML"));
        assert!(is_xml_file("3D/3dmodel.MODEL"));
        assert!(is_xml_file("_rels/.rels"));
        assert!(!is_xml_file("textures/tex.png"));
    }

    #[test]
    fn model_file_detection_is_case_insensitive() {
        assert!(is_model_file("3D/3dmodel.MODEL"));
        assert!(!is_model_file("3D/3dmodel.xml"));
    }
}
